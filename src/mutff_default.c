...
#define MuTFF_FN(func, ...)               \
  do {                                    \
    err = func(ctx, &bytes, __VA_ARGS__); \
...
static MuTFFError mutff_read_u8(MuTFFContext *ctx, size_t *n, uint8_t *data) {
  const MuTFFError err = mutff_read(ctx, data, 1);
  if (err != MuTFFErrorNone) {
    return err;
  }
  *n = 1U;
...
static MuTFFError mutff_read_i8(MuTFFContext *ctx, size_t *n, int8_t *dest) {
  MuTFFError err;
  uint8_t twos;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_read_u16(MuTFFContext *ctx, size_t *n, uint16_t *dest) {
  unsigned char data[2];
  const MuTFFError err = mutff_read(ctx, data, 2);
  if (err != MuTFFErrorNone) {
    return err;
  }
...
static MuTFFError mutff_read_i16(MuTFFContext *ctx, size_t *n, int16_t *dest) {
  MuTFFError err;
  uint16_t twos;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_read_u24(MuTFFContext *ctx, size_t *n,
                                 mutff_uint24_t *dest) {
  unsigned char data[3];
  const MuTFFError err = mutff_read(ctx, data, 3);
  if (err != MuTFFErrorNone) {
    return err;
  }
...
static MuTFFError mutff_read_u32(MuTFFContext *ctx, size_t *n, uint32_t *dest) {
  unsigned char data[4];
  const MuTFFError err = mutff_read(ctx, data, 4);
  if (err != MuTFFErrorNone) {
    return err;
  }
...
static MuTFFError mutff_read_i32(MuTFFContext *ctx, size_t *n, int32_t *dest) {
  MuTFFError err;
  uint32_t twos;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_read_u64(MuTFFContext *ctx, size_t *n, uint64_t *dest) {
  unsigned char data[8];
  const MuTFFError err = mutff_read(ctx, data, 8);
  if (err != MuTFFErrorNone) {
    return err;
  }
...
static MuTFFError mutff_write_u8(MuTFFContext *ctx, size_t *n, uint8_t data) {
  const MuTFFError err = mutff_write(ctx, &data, 1);
  if (err != MuTFFErrorNone) {
    return err;
  }
  *n = 1U;
...
static MuTFFError mutff_write_i8(MuTFFContext *ctx, size_t *n, int8_t x) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_write_u16(MuTFFContext *ctx, size_t *n, uint16_t x) {
  unsigned char data[2];
  // convert number to network order
  mutff_hton_16(data, x);
  const MuTFFError err = mutff_write(ctx, &data, 2);
  if (err != MuTFFErrorNone) {
    return err;
  }
  *n = 2U;
...
static inline MuTFFError mutff_write_i16(MuTFFContext *ctx, size_t *n,
                                         int16_t x) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_write_u24(MuTFFContext *ctx, size_t *n,
                                  mutff_uint24_t x) {
  unsigned char data[3];
  mutff_hton_24(data, x);
  const MuTFFError err = mutff_write(ctx, &data, 3);
  if (err != MuTFFErrorNone) {
    return err;
  }
  *n = 3U;
...
static MuTFFError mutff_write_u32(MuTFFContext *ctx, size_t *n, uint32_t x) {
  unsigned char data[4];
  mutff_hton_32(data, x);
  const MuTFFError err = mutff_write(ctx, &data, 4);
  if (err != MuTFFErrorNone) {
    return err;
  }
  *n = 4U;
...
static inline MuTFFError mutff_write_i32(MuTFFContext *ctx, size_t *n,
                                         int32_t x) {
...
static MuTFFError mutff_read_q8_8(MuTFFContext *ctx, size_t *n,
                                  mutff_q8_8_t *data) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_write_q8_8(MuTFFContext *ctx, size_t *n,
                                   mutff_q8_8_t data) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_read_q16_16(MuTFFContext *ctx, size_t *n,
                                    mutff_q16_16_t *data) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_write_q16_16(MuTFFContext *ctx, size_t *n,
                                     mutff_q16_16_t data) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_read_q2_30(MuTFFContext *ctx, size_t *n,
                                   mutff_q2_30_t *data) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_write_q2_30(MuTFFContext *ctx, size_t *n,
                                    mutff_q2_30_t data) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_read_matrix(MuTFFContext *ctx, size_t *n,
                                    MuTFFMatrix *matrix) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_write_matrix(MuTFFContext *ctx, size_t *n,
                                     MuTFFMatrix matrix) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_read_header(MuTFFContext *ctx, size_t *n,
...
static MuTFFError mutff_peek_atom_header(MuTFFContext *ctx, size_t *n,
                                         uint64_t *size, uint32_t *type) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
static MuTFFError mutff_write_header(MuTFFContext *ctx, size_t *n,
                                     uint64_t size, uint32_t type) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_read_quickdraw_rect(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_quickdraw_rect(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_quickdraw_region(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_quickdraw_region(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_file_type_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_file_type_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_movie_data_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_movie_data_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_free_atom(MuTFFContext *ctx, size_t *n,
                                MuTFFFreeAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_write_free_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_skip_atom(MuTFFContext *ctx, size_t *n,
                                MuTFFSkipAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_write_skip_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_wide_atom(MuTFFContext *ctx, size_t *n,
                                MuTFFWideAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_write_wide_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_preview_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_preview_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_movie_header_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_movie_header_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_clipping_region_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_clipping_region_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_clipping_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_clipping_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_color_table_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_color_table_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_user_data_list_entry(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_user_data_list_entry(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_user_data_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_user_data_atom(MuTFFContext *ctx, size_t *n,
...
static inline MuTFFError mutff_movie_extends_header_atom_size(
    uint64_t *out, const MuTFFMovieExtendsHeaderAtom *atom) {
  switch (atom->version) {
    case 0:
...
MuTFFError mutff_write_movie_extends_header_atom(
  ...
  *n = 0;
  uint64_t size;
  err = mutff_movie_extends_header_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
  MuTFF_FN(mutff_write_header, size, MuTFF_FOURCC('m', 'e', 'h', 'd'));
  MuTFF_FN(mutff_write_u8, in->version);
  MuTFF_FN(mutff_write_u24, in->flags);
  switch (in->version) {
    case 0:
...
static inline MuTFFError mutff_track_extends_atom_size(
    uint64_t *out, const MuTFFTrackExtendsAtom *atom) {
...
MuTFFError mutff_write_track_extends_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  uint64_t size;
  err = mutff_track_extends_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
...
MuTFFError mutff_read_movie_extends_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  ...
  while (*n < size) {
    ...
    switch (child_type) {
      case MuTFF_FOURCC('m', 'e', 'h', 'd'):
        MuTFF_READ_CHILD(mutff_read_movie_extends_header_atom,
                         &out->movie_extends_header,
...
static inline MuTFFError mutff_movie_extends_atom_size(
    uint64_t *out, const MuTFFMovieExtendsAtom *atom) {
  MuTFFError err;
  uint64_t child_size;
  *out = 0;
  if (atom->movie_extends_header_present) {
    err = mutff_movie_extends_header_atom_size(&child_size,
                                               &atom->movie_extends_header);
    if (err != MuTFFErrorNone) {
      return err;
    }
    *out += child_size;
...
MuTFFError mutff_write_movie_extends_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  uint64_t size;
  err = mutff_movie_extends_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
...
MuTFFError mutff_read_track_header_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_track_header_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_track_clean_aperture_dimensions_atom(
    MuTFFContext *ctx, size_t *n, MuTFFTrackCleanApertureDimensionsAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_track_encoded_pixels_dimensions_atom(
    MuTFFContext *ctx, size_t *n, MuTFFTrackEncodedPixelsDimensionsAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_track_aperture_mode_dimensions_atom(
    MuTFFContext *ctx, size_t *n, MuTFFTrackApertureModeDimensionsAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_sample_description(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_sample_description(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_video_sample_description(
    MuTFFContext *ctx, size_t *n, MuTFFVideoSampleDescription *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  MuTFF_FN(mutff_read_u16, &out->version);
...
MuTFFError mutff_write_video_sample_description(
    MuTFFContext *ctx, size_t *n, const MuTFFVideoSampleDescription *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  MuTFF_FN(mutff_write_u16, in->version);
...
MuTFFError mutff_read_compressed_matte_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_compressed_matte_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFCompressedMatteAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_track_matte_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_track_matte_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_edit_list_entry(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_edit_list_entry(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_edit_list_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_edit_list_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_edit_atom(MuTFFContext *ctx, size_t *n,
                                MuTFFEditAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_write_edit_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_track_reference_type_atom(
    MuTFFContext *ctx, size_t *n, MuTFFTrackReferenceTypeAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_track_reference_type_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFTrackReferenceTypeAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_track_reference_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_track_reference_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_track_exclude_from_autoselection_atom(
    MuTFFContext *ctx, size_t *n, MuTFFTrackExcludeFromAutoselectionAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_track_load_settings_atom(
    MuTFFContext *ctx, size_t *n, MuTFFTrackLoadSettingsAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_track_load_settings_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFTrackLoadSettingsAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_input_type_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_input_type_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_object_id_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_object_id_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_track_input_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_track_input_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_track_input_map_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_track_input_map_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_media_header_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_media_header_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_extended_language_tag_atom(
    MuTFFContext *ctx, size_t *n, MuTFFExtendedLanguageTagAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_extended_language_tag_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFExtendedLanguageTagAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  size_t i;
...
MuTFFError mutff_read_handler_reference_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_handler_reference_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFHandlerReferenceAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_video_media_information_header_atom(
    MuTFFContext *ctx, size_t *n, MuTFFVideoMediaInformationHeaderAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_data_reference(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_data_reference(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_data_reference_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_data_reference_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_data_information_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_data_information_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFDataInformationAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_sample_description_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_sample_description_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFSampleDescriptionAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_time_to_sample_table_entry(
    MuTFFContext *ctx, size_t *n, MuTFFTimeToSampleTableEntry *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_write_time_to_sample_table_entry(
    MuTFFContext *ctx, size_t *n, const MuTFFTimeToSampleTableEntry *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_read_time_to_sample_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_time_to_sample_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_composition_offset_table_entry(
    MuTFFContext *ctx, size_t *n, MuTFFCompositionOffsetTableEntry *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  MuTFF_FN(mutff_read_u32, &out->sample_count);
...
MuTFFError mutff_write_composition_offset_table_entry(
    MuTFFContext *ctx, size_t *n, const MuTFFCompositionOffsetTableEntry *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  MuTFF_FN(mutff_write_u32, in->sample_count);
...
MuTFFError mutff_read_composition_offset_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_composition_offset_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFCompositionOffsetAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_composition_shift_least_greatest_atom(
    MuTFFContext *ctx, size_t *n, MuTFFCompositionShiftLeastGreatestAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_sync_sample_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_sync_sample_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_partial_sync_sample_atom(
    MuTFFContext *ctx, size_t *n, MuTFFPartialSyncSampleAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_partial_sync_sample_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFPartialSyncSampleAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_sample_to_chunk_table_entry(
    MuTFFContext *ctx, size_t *n, MuTFFSampleToChunkTableEntry *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_write_sample_to_chunk_table_entry(
    MuTFFContext *ctx, size_t *n, const MuTFFSampleToChunkTableEntry *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_read_sample_to_chunk_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_sample_to_chunk_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_sample_size_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_sample_size_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_chunk_offset_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_chunk_offset_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_sample_dependency_flags_atom(
    MuTFFContext *ctx, size_t *n, MuTFFSampleDependencyFlagsAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_sample_dependency_flags_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFSampleDependencyFlagsAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_sample_table_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_sample_table_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_video_media_information_atom(
    MuTFFContext *ctx, size_t *n, MuTFFVideoMediaInformationAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_video_media_information_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFVideoMediaInformationAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_read_sound_media_information_header_atom(
    MuTFFContext *ctx, size_t *n, MuTFFSoundMediaInformationHeaderAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_sound_media_information_atom(
    MuTFFContext *ctx, size_t *n, MuTFFSoundMediaInformationAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_sound_media_information_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFSoundMediaInformationAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_read_base_media_info_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_write_base_media_info_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_text_media_information_atom(
    MuTFFContext *ctx, size_t *n, MuTFFTextMediaInformationAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_text_media_information_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFTextMediaInformationAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_base_media_information_header_atom(
    MuTFFContext *ctx, size_t *n, MuTFFBaseMediaInformationHeaderAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_base_media_information_atom(
    MuTFFContext *ctx, size_t *n, MuTFFBaseMediaInformationAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_write_base_media_information_atom(
    MuTFFContext *ctx, size_t *n, const MuTFFBaseMediaInformationAtom *in) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  uint64_t size;
...
MuTFFError mutff_read_media_atom(MuTFFContext *ctx, size_t *n,
                                 MuTFFMediaAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
  ...
  while (*n < size) {
    ...
    switch (child_type) {
      ...
      case MuTFF_FOURCC('m', 'i', 'n', 'f'):
        ...
        err = mutff_tell(ctx, &media_information_offset);
        if (err != MuTFFErrorNone) {
          return err;
        }
        MuTFF_SEEK_CUR(child_size);
  ...
  err = mutff_tell(ctx, &atom_end_offset);
  if (err != MuTFFErrorNone) {
    return err;
  }
  ...
  if (out->media_information_present) {
    ...
    err =
        mutff_seek(ctx, (long)media_information_offset - (long)atom_end_offset);
    if (err != MuTFFErrorNone) {
      return err;
    }
    ...
    err = mutff_seek(ctx, (long)atom_end_offset -
...
MuTFFError mutff_write_media_atom(MuTFFContext *ctx, size_t *n,
...
MuTFFError mutff_read_track_atom(MuTFFContext *ctx, size_t *n,
                                 MuTFFTrackAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_read_movie_atom(MuTFFContext *ctx, size_t *n,
                                 MuTFFMovieAtom *out) {
  MuTFFError err;
  size_t bytes;
  *n = 0;
...
MuTFFError mutff_read_movie_fragment_header_atom(
  ...
  *n = 0;
  ...
  uint32_t type;
  MuTFF_FN(mutff_read_header, &size, &type);
  if (type != MuTFF_FOURCC('m', 'f', 'h', 'd')) {
    return MuTFFErrorBadFormat;
  }
  MuTFF_FN(mutff_read_u8, &out->version);
  MuTFF_FN(mutff_read_u24, &out->flags);
  MuTFF_FN(mutff_read_u32, &out->sequence_number);
  MuTFF_SEEK_CUR(size - *n);
  return MuTFFErrorNone;
}

static inline MuTFFError mutff_movie_fragment_header_atom_size(
    uint64_t *out, const MuTFFMovieFragmentHeaderAtom *atom) {
  *out = mutff_atom_size(8);
  return MuTFFErrorNone;
}
...
MuTFFError mutff_write_movie_fragment_header_atom(
  ...
  *n = 0;
  uint64_t size;
  err = mutff_movie_fragment_header_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
  MuTFF_FN(mutff_write_header, size, MuTFF_FOURCC('m', 'f', 'h', 'd'));
  MuTFF_FN(mutff_write_u8, in->version);
  MuTFF_FN(mutff_write_u24, in->flags);
  MuTFF_FN(mutff_write_u32, in->sequence_number);
  return MuTFFErrorNone;
}
...
MuTFFError mutff_read_track_fragment_header_atom(
  ...
  *n = 0;
  ...
  MuTFF_FN(mutff_read_header, &size, &type);
  if (type != MuTFF_FOURCC('t', 'f', 'h', 'd')) {
    return MuTFFErrorBadFormat;
  }
  MuTFF_FN(mutff_read_u8, &version);
  if (version != 0U) {
    return MuTFFErrorBadFormat;
  }
  MuTFF_FN(mutff_read_u24, &flags);
  out->base_data_offset_present = flags & 0x000001U;
  out->sample_description_index_present = flags & 0x000002U;
  out->default_sample_duration_present = flags & 0x000008U;
  out->default_sample_size_present = flags & 0x000010U;
  out->default_sample_flags_present = flags & 0x000020U;
  out->duration_is_empty = flags & 0x010000U;
  out->default_base_is_moof = flags & 0x020000U;
  MuTFF_FN(mutff_read_u32, &out->track_id);
  if (out->base_data_offset_present) {
    MuTFF_FN(mutff_read_u64, &out->base_data_offset);
  }
  if (out->sample_description_index_present) {
    MuTFF_FN(mutff_read_u32, &out->sample_description_index);
  }
  if (out->default_sample_duration_present) {
    MuTFF_FN(mutff_read_u32, &out->default_sample_duration);
  }
  if (out->default_sample_size_present) {
    MuTFF_FN(mutff_read_u32, &out->default_sample_size);
  }
  if (out->default_sample_flags_present) {
    MuTFF_FN(mutff_read_u32, &out->default_sample_flags);
  }
  MuTFF_SEEK_CUR(size - *n);
  return MuTFFErrorNone;
...
static inline MuTFFError mutff_track_fragment_header_atom_size(
    uint64_t *out, const MuTFFTrackFragmentHeaderAtom *atom) {
  *out = 8;
  if (atom->base_data_offset_present) {
    *out += 8;
  }
  if (atom->sample_description_index_present) {
    *out += 4;
  }
  if (atom->default_sample_duration_present) {
    *out += 4;
  }
  if (atom->default_sample_size_present) {
    *out += 4;
  }
  if (atom->default_sample_flags_present) {
    *out += 4;
  }
  *out = mutff_atom_size(*out);
  return MuTFFErrorNone;
...
MuTFFError mutff_write_track_fragment_header_atom(
  ...
  size_t bytes;
  *n = 0;
  mutff_uint24_t flags = 0U;
  uint64_t size;
  err = mutff_track_fragment_header_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
  MuTFF_FN(mutff_write_header, size, MuTFF_FOURCC('t', 'f', 'h', 'd'));
  MuTFF_FN(mutff_write_u8, 0);
  flags = 0;
  if (in->base_data_offset_present) {
    flags |= 0x000001U;
  }
  if (in->sample_description_index_present) {
    flags |= 0x000002U;
  }
  if (in->default_sample_duration_present) {
    flags |= 0x000008U;
  }
  if (in->default_sample_size_present) {
    flags |= 0x000010U;
  }
  if (in->default_sample_flags_present) {
    flags |= 0x000020U;
  }
  if (in->duration_is_empty) {
    flags |= 0x010000U;
  }
  if (in->default_base_is_moof) {
    flags |= 0x020000U;
  }
  MuTFF_FN(mutff_write_u24, flags);
  MuTFF_FN(mutff_write_u32, in->track_id);
  if (in->base_data_offset_present) {
    MuTFF_FN(mutff_write_u64, in->base_data_offset);
  }
  if (in->sample_description_index_present) {
    MuTFF_FN(mutff_write_u32, in->sample_description_index);
  }
  if (in->default_sample_duration_present) {
    MuTFF_FN(mutff_write_u32, in->default_sample_duration);
  }
...
MuTFFError mutff_read_track_fragment_run_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  ...
  MuTFF_FN(mutff_read_header, &size, &type);
  if (type != MuTFF_FOURCC('t', 'r', 'u', 'n')) {
    return MuTFFErrorBadFormat;
  }
  MuTFF_FN(mutff_read_u8, &out->version);
  MuTFF_FN(mutff_read_u24, &flags);
  out->data_offset_present = flags & 0x01U;
  out->first_sample_flags_present = flags & 0x04U;
  out->sample_duration_present = flags & 0x100U;
  out->sample_size_present = flags & 0x200U;
  out->sample_flags_present = flags & 0x400U;
  out->sample_composition_time_offset_present = flags & 0x800U;
  MuTFF_FN(mutff_read_u32, &out->sample_count);
  if (out->data_offset_present) {
    MuTFF_FN(mutff_read_i32, &out->data_offset);
  }
  if (out->first_sample_flags_present) {
    MuTFF_FN(mutff_read_u32, &out->first_sample_flags);
  }
  ...
  if (out->sample_count * record_size > size - *n) {
    return MuTFFErrorBadFormat;
  }
...
static inline MuTFFError mutff_track_fragment_run_atom_size(
    uint64_t *out, const MuTFFTrackFragmentRunAtom *atom) {
  *out = 8U;

  if (atom->data_offset_present) {
    *out += 4U;
  }
  if (atom->first_sample_flags_present) {
    *out += 4U;
  }
  if (atom->sample_duration_present) {
    *out += 4U * atom->sample_count;
  }
  if (atom->sample_size_present) {
    *out += 4U * atom->sample_count;
  }
  if (atom->sample_flags_present) {
    *out += 4U * atom->sample_count;
  }
  if (atom->sample_composition_time_offset_present) {
    *out += 4U * atom->sample_count;
  }
...
MuTFFError mutff_write_track_fragment_run_atom(
  ...
  MuTFFError err;
  size_t bytes;
  mutff_uint24_t flags = 0;
  *n = 0;

  uint64_t size;
  err = mutff_track_fragment_run_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
  MuTFF_FN(mutff_write_header, size, MuTFF_FOURCC('t', 'r', 'u', 'n'));
  MuTFF_FN(mutff_write_u8, in->version);
  if (in->data_offset_present) {
    flags |= 0x01U;
  }
  if (in->first_sample_flags_present) {
    flags |= 0x04U;
  }
  if (in->sample_duration_present) {
    flags |= 0x100U;
  }
  if (in->sample_size_present) {
    flags |= 0x200U;
  }
  if (in->sample_flags_present) {
    flags |= 0x400U;
  }
  if (in->sample_composition_time_offset_present) {
    flags |= 0x800U;
  }
  MuTFF_FN(mutff_write_u24, flags);
  MuTFF_FN(mutff_write_u32, in->sample_count);
  ...
  if (in->first_sample_flags_present) {
    MuTFF_FN(mutff_write_u32, in->first_sample_flags);
  }

  for (uint32_t i = 0; i < in->sample_count; ++i) {
    if (in->sample_duration_present) {
      MuTFF_FN(mutff_write_u32, in->records[i].sample_duration);
    }
    if (in->sample_size_present) {
      MuTFF_FN(mutff_write_u32, in->records[i].sample_size);
    }
    if (in->sample_flags_present) {
      MuTFF_FN(mutff_write_u32, in->records[i].sample_flags);
    }
    if (in->sample_composition_time_offset_present) {
      if (in->version == 0U) {
        if (in->records[i].sample_composition_time_offset < 0) {
          return MuTFFErrorBadFormat;
        }
        MuTFF_FN(mutff_write_u32,
                 in->records[i].sample_composition_time_offset);
      } else {
...
MuTFFError mutff_read_track_fragment_decode_time_atom(
  ...
  *n = 0;
  ...
  if (out->version == 1U) {
    ...
    MuTFF_FN(mutff_read_u32, &base_media_decode_time);
    out->base_media_decode_time = base_media_decode_time;
  }

  return MuTFFErrorNone;
}

static inline MuTFFError mutff_track_fragment_decode_time_atom_size(
    uint64_t *out, const MuTFFTrackFragmentDecodeTimeAtom *atom) {
  *out = 4U;
  if (atom->version == 1U) {
    *out += 8U;
  } else {
...
MuTFFError mutff_write_track_fragment_decode_time_atom(
  ...
  *n = 0;
  ...
  uint64_t size;
  err = mutff_track_fragment_decode_time_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
  MuTFF_FN(mutff_write_header, size, MuTFF_FOURCC('t', 'f', 'd', 't'));
  MuTFF_FN(mutff_write_u8, in->version);
  MuTFF_FN(mutff_write_u24, 0U);
  if (in->version == 1U) {
    MuTFF_FN(mutff_write_u64, in->base_media_decode_time);
  } else {
    if (in->base_media_decode_time > UINT32_MAX) {
      return MuTFFErrorBadFormat;
    }
    MuTFF_FN(mutff_write_u32, in->base_media_decode_time);
  }
...
MuTFFError mutff_read_track_fragment_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  uint64_t size;
  uint32_t type;
  bool track_fragment_header_present = false;

  out->track_fragment_run_count = 0;
  out->track_fragment_decode_time_present = false;
  out->user_data_present = false;

  MuTFF_FN(mutff_read_header, &size, &type);
  if (type != MuTFF_FOURCC('t', 'r', 'a', 'f')) {
    return MuTFFErrorBadFormat;
  }
  ...
  while (*n < size) {
    ...
    switch (child_type) {
      case MuTFF_FOURCC('t', 'f', 'h', 'd'):
        MuTFF_READ_CHILD(mutff_read_track_fragment_header_atom,
                         &out->track_fragment_header,
                         track_fragment_header_present);
        break;

      case MuTFF_FOURCC('t', 'r', 'u', 'n'):
        if (out->track_fragment_run_count >=
            MuTFF_MAX_TRACK_FRAGMENT_RUN_ATOMS) {
          return MuTFFErrorBadFormat;
        }
        MuTFF_FN(mutff_read_track_fragment_run_atom,
                 &out->track_fragment_run[out->track_fragment_run_count]);
        out->track_fragment_run_count++;
        break;

      case MuTFF_FOURCC('t', 'f', 'd', 't'):
        MuTFF_READ_CHILD(mutff_read_track_fragment_decode_time_atom,
                         &out->track_fragment_decode_time,
                         out->track_fragment_decode_time_present);
        break;
  ...
  if (!track_fragment_header_present) {
    return MuTFFErrorBadFormat;
  }
...
static inline MuTFFError mutff_track_fragment_atom_size(
    uint64_t *out, const MuTFFTrackFragmentAtom *atom) {
  MuTFFError err;
  uint64_t size;
  uint64_t child_size;
  err = mutff_track_fragment_header_atom_size(&size,
                                              &atom->track_fragment_header);
  if (err != MuTFFErrorNone) {
    return err;
  }
  for (size_t i = 0; i < atom->track_fragment_run_count; ++i) {
    err = mutff_track_fragment_run_atom_size(&child_size,
                                             &atom->track_fragment_run[i]);
    if (err != MuTFFErrorNone) {
      return err;
    }
    size += child_size;
  }
  if (atom->track_fragment_decode_time_present) {
    err = mutff_track_fragment_decode_time_atom_size(
        &child_size, &atom->track_fragment_decode_time);
    if (err != MuTFFErrorNone) {
      return err;
    }
    size += child_size;
...
MuTFFError mutff_write_track_fragment_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  ...
  uint64_t size;
  err = mutff_track_fragment_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
  ...
  if (in->track_fragment_decode_time_present) {
    MuTFF_FN(mutff_write_track_fragment_decode_time_atom,
             &in->track_fragment_decode_time);
  }
  if (in->user_data_present) {
    MuTFF_FN(mutff_write_user_data_atom, &in->user_data);
  }
...
MuTFFError mutff_read_movie_fragment_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  uint64_t size;
  uint32_t type;
  bool movie_fragment_header_present = false;

  out->track_fragment_count = 0;
  out->user_data_present = false;

  MuTFF_FN(mutff_read_header, &size, &type);
  if (type != MuTFF_FOURCC('m', 'o', 'o', 'f')) {
    return MuTFFErrorBadFormat;
  }
  ...
  while (*n < size) {
    ...
    switch (child_type) {
      case MuTFF_FOURCC('m', 'f', 'h', 'd'):
        MuTFF_READ_CHILD(mutff_read_movie_fragment_header_atom,
                         &out->movie_fragment_header,
                         movie_fragment_header_present);
        break;

      case MuTFF_FOURCC('t', 'r', 'a', 'f'):
        if (out->track_fragment_count >= MuTFF_MAX_TRACK_FRAGMENT_ATOMS) {
          return MuTFFErrorBadFormat;
        }
        MuTFF_FN(mutff_read_track_fragment_atom,
                 &out->track_fragment[out->track_fragment_count]);
        out->track_fragment_count++;
        break;
  ...
  if (!movie_fragment_header_present) {
    return MuTFFErrorBadFormat;
  }
...
static inline MuTFFError mutff_movie_fragment_atom_size(
    uint64_t *out, const MuTFFMovieFragmentAtom *atom) {
  MuTFFError err;
  uint64_t size;
  uint64_t child_size;
  err = mutff_movie_fragment_header_atom_size(&size,
                                              &atom->movie_fragment_header);
  if (err != MuTFFErrorNone) {
    return err;
  }
  for (size_t i = 0; i < atom->track_fragment_count; ++i) {
    err = mutff_track_fragment_atom_size(&child_size, &atom->track_fragment[i]);
    if (err != MuTFFErrorNone) {
      return err;
    }
    size += child_size;
...
MuTFFError mutff_write_movie_fragment_atom(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  ...
  uint64_t size;
  err = mutff_movie_fragment_atom_size(&size, in);
  if (err != MuTFFErrorNone) {
    return err;
  }
  MuTFF_FN(mutff_write_header, size, MuTFF_FOURCC('m', 'o', 'o', 'f'));
  MuTFF_FN(mutff_write_movie_fragment_header_atom, &in->movie_fragment_header);
  for (size_t i = 0; i < in->track_fragment_count; ++i) {
    MuTFF_FN(mutff_write_track_fragment_atom, &in->track_fragment[i]);
  }
  if (in->user_data_present) {
    MuTFF_FN(mutff_write_user_data_atom, &in->user_data);
  }
...
MuTFFError mutff_read_movie_file(MuTFFContext *ctx, size_t *n,
                                 MuTFFMovieFile *out) {
  MuTFFError err;
  uint64_t size;
  uint32_t type;
  ...
  *n = 0;
  ...
  while (mutff_peek_atom_header(ctx, &bytes, &size, &type) == MuTFFErrorNone) {
    if (size == 0U) {
      return MuTFFErrorBadFormat;
    }
    ...
    switch (type) {
      ...
      case MuTFF_FOURCC('m', 'o', 'o', 'f'):
        if (out->movie_fragment_count >= MuTFF_MAX_MOVIE_FRAGMENT_ATOMS) {
          return MuTFFErrorOutOfMemory;
        }
        MuTFF_FN(mutff_read_movie_fragment_atom,
                 &out->movie_fragment[out->movie_fragment_count]);
        out->movie_fragment_count++;
        break;
...
MuTFFError mutff_write_movie_file(MuTFFContext *ctx, size_t *n,
  ...
  *n = 0;
  ...
  for (size_t i = 0; i < in->movie_fragment_count; ++i) {
    MuTFF_FN(mutff_write_movie_fragment_atom, &in->movie_fragment[i]);
  }
